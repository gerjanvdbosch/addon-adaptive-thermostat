...
printf 'adaptive_thermostat started at %s\nMODEL_DIR=%s\n' "$(date --iso-8601=seconds)" "$MODEL_DIR" >> "$MODEL_DIR/general.log" 2>/dev/null || true

# Start WSGI entry (api_service exposes app via run_proc.py)
exec python3 -u /opt/adaptive_thermostat/app/run_proc.py

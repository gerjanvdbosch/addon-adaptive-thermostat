...
class SolarAI:
    ...
    def calculate_scenario(
        self, df: pd.DataFrame, custom_bias: float = None
    ) -> pd.DataFrame:
        """
        Past de volledige SolarAI logica toe op een historische dataset.
        Berekent: AI Prediction, Drempelwaardes, Status (Start/Wait).
        """
        df = df.copy()  # Werk op een kopie, verander het origineel niet

        # 1. Bepaal de Bias (Gebruik huidige bias als er geen specifieke is opgegeven)
        bias = custom_bias if custom_bias is not None else self.smoothed_bias

        # 2. AI Prediction (Blending: Model + Solcast)
        # Check eerst of we features moeten maken
        if "pv_estimate" in df.columns:
            # Als het model getraind is, gebruik blending
            if self.is_fitted and self.model:
                X_feat = self._create_features(df)
                pred_ai = self.model.predict(X_feat)
                # Blending (gebruik class weights of fallback 0.6/0.4)
                w_ml = getattr(self, "ml_weight", 0.6)
                w_sol = getattr(self, "solcast_weight", 0.4)
                raw_power = (w_ml * pred_ai) + (w_sol * df["pv_estimate"])
            else:
                raw_power = df["pv_estimate"]

            # Pas bias toe en clip
            df["ai_pred"] = (raw_power * bias).clip(0, self.system_max_kw)

        # 3. Seizoensfactor Bepalen
        # Pak de dag van het jaar uit de eerste timestamp
        if not df.empty:
            doy = df.index[0].dayofyear
        else:
            doy = 180
        ...
        # 4. Dag Totalen (nodig voor Remaining Ratio)
        total_energy = df["ai_pred"].sum()
        day_peak = df["ai_pred"].max()

        day_quality_ratio = day_peak / max(0.1, seasonal_max_kw)
        day_quality_ratio = min(day_quality_ratio, 1.0)

        # Minima
        day_floor = day_peak * 0.25
        min_viable = self.min_viable_kw

        # 5. Loop voor Thresholds (Dit bootst het verloop van de tijd na)
        thresholds = []
        statuses = []

        for idx, row in df.iterrows():
            # Bereken hoeveel energie er nog komt NA dit moment
            future_sum = df.loc[idx:]["ai_pred"].sum()

            remaining_ratio = (future_sum / total_energy) if total_energy > 0 else 0.0

            # De Formule: Kwaliteit + Geduld
            pct = 0.60 + (day_quality_ratio * 0.20) + (remaining_ratio * 0.15)
            pct = max(0.65, min(pct, 0.90))

            # Drempel bepalen
            thr = max(day_peak * pct, day_floor, min_viable)
            thresholds.append(thr)

            # Status bepalen (Simulatie van besluitvorming)
            # Als Actual > Threshold -> START
            if pd.notna(row.get("actual_pv_yield")) and row["actual_pv_yield"] > thr:

...
class ThermalAI:
    ...
    def __init__(self, ha_client: HAClient, opts: dict):
        ...
        self.min_cycle_minutes = 45  # Vloerverwarming heeft tijd nodig om massa te activeren
        self.dead_time_minutes = 20  # Tijd voordat de vloer effect heeft op de luchttemperatuur
        ...
        # EINDE: Warmtepomp stopt (Target bereikt of overshoot fase)
            ...
            if self.cycle_start_ts and self.start_temp is not None:
                ...
                if self.min_cycle_minutes < duration_min < 600 and temp_delta > 0.05:
                    logger.info(f"ThermalAI: Cyclus voltooid: +{temp_delta:.2f}°C in {duration_min:.0f} min.")
    ...
    def train(self):
        ...
        try:
            ...
            logger.info(f"ThermalAI: Training klaar. Gemiddelde stijging: {avg_rate_hour:.2f}°C/uur.")
    ...
    def predict_heating_time(self, target_temp):
        ...
        X_pred = pd.DataFrame([[current_temp, outside_temp]],
                              columns=["start_temp", "outside_temp"])
